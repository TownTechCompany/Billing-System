...
class OrderService:
    ...
    def create(self, items: list, payment_method: str = "Cash"):
        order_num = f"ORD-{int(datetime.utcnow().timestamp())}"
        total = sum(i["price"] * i["quantity"] for i in items)
        order = Order(order_number=order_num, total_amount=total, payment_method=payment_method)
    ...
    def analytics(self):
        rows = (
            self.db.query(func.date(Order.date_created).label("date"), func.sum(Order.total_amount).label("total"))
            .group_by(func.date(Order.date_created))
            .order_by(func.date(Order.date_created))
            .limit(7).all()
        )
        return {"labels": [r.date for r in rows] or ["No Data"], "data": [r.total for r in rows] or [0]}
